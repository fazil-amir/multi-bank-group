...
Production-ready full-stack crypto price dashboard: live spot prices and historical charts for 14 USDT pairs, and real-time updates via Server-Sent Events.
...
```bash
docker run -d -p 4000:4000 -e NODE_ENV=production \
...
┌──────────────────────────────────────────────────────────────────┐
│                         Browser (SPA)                            │
│  React + Vite bundle  │  TrackerDataContext                      │
│  SSE client (2s)      │  Cache API (history)                     │
└──────────────────────────────┬───────────────────────────────────┘
                               │ HTTP/SSE (same origin or CORS)
┌──────────────────────────────▼───────────────────────────────────┐
│                    Express (single process)                      │
│  /api/trackers/symbols  │  /api/trackers/streams                 │
...
- **Bootstrap:** Server starts → `connectTracker()` opens Binance WebSocket → price map populated → Express serves API + static. Client loads → `TrackerDataProvider` (trackers + SSE).
...
Uses image `fazilamir/crypto-dashboard:latest`, port `4000`, `NODE_ENV=production`.
...
Put the app behind nginx, Caddy, or Traefik with HTTPS.
...
|--------|------|-------------|
| `GET` | `/api/trackers/symbols` | Returns list of tracker configs (id, name, symbol, icon, etc.). |
| `GET` | `/api/trackers/streams` | SSE stream; JSON array of live prices every 2s. |
| `GET` | `/api/trackers/:symbol` | Historical OHLC for symbol (1D, ~1y). Cached 24h server-side. |
...
│   │   ├── main.tsx           # React root
│   │   ├── app/App.tsx        # Routes: /*
│   │   ├── contexts/          # TrackerDataContext
│   │   ├── pages/             # TrackersPage, TrackerDetailPage
│   │   ├── components/        # tracker-*, ui/table
│   │   ├── hooks/             # useLivePrices (SSE), useHistory, useTrackers
│   │   └── ui/
│   ├── server/                # Express API
│   │   ├── index.ts           # App setup, static, connectTracker()
│   │   ├── routes/            # trackers
│   │   ├── controllers/       # trackers (SSE, history)
│   │   └── services/          # trackers (WS), history (klines + cache)
│   └── shared/                # Shared by client + server
│       ├── constants/         # app, api, tracker, trackerSymbols
│       ├── types/             # market.types
...
- **CORS:** Restrict `origin` in production if needed.
...
| Area | Enhancement |
|------|-------------|
| **API protection** | **Rate limiting** — Per-IP limits on API endpoints (e.g. express-rate-limit) to reduce abuse. |
| **Alerts** | **Price alerts** — Let users set target prices; notify (in-app, email, or webhook) when price is reached; store in DB. |
...
1. **Trackers:** `useTrackers()` → GET `/api/trackers/symbols` → list of symbols/config.
2. **Live prices:** Server: Binance WS → in-memory map. SSE handler sends snapshot every 2s. Client: `useLivePrices()` subscribes to SSE, buffers, flushes every 2s, computes trends.
3. **History:** GET `/api/trackers/:symbol` → server cache or Binance klines → client caches in Cache API (24h).
...
| **SSE / prices not updating** | Check server logs; ensure Binance WebSocket is connected and `/api/trackers/streams` is reachable. |
